...
def demo_preprocessing():
    '''
    Demos how our preprocessing works when it comes to extracting features
    This is done for both test data and training data
    The training data we have in the folder is not all we worked with, but for sake of time I only wanted to demo it on one face
    '''
    import dlib
    import glob
    '''
        First part - finding out what faces can be preprocessed and excluding those that can't be recognized
        If it can't be recognized then it doesn't give any features and therefore would be mostly useless
        
    '''
    data_directory = get_data_directory()
    predictor_path = data_directory + "/dlib-models/shape_predictor_68_face_landmarks.dat"
    faces_folder_path = data_directory + "\\training"
    ...
    # If it is a 1 in the list then it was excluded and a 0 means included
    # Going to be used when grabbing labels and when figuring out what to bound
    # Leaving this in to show our method of exclusion
    # We just check if the index is 0 or 1 with 1 being exclude and 1 being include
    # exclusion_list = []
    rect = []
    for f in glob.glob(os.path.join(faces_folder_path, "*.jpg")):
        print("Processing file: {}".format(f))
        ...
        '''
            Second part - Cropping the image based on the bounding box
        '''

        # rect = d
        item_location = data_directory + "/training/" + "0000_00000001.jpg"
        im = Image.open(item_location).convert('L')
        im = im.crop((rect.left(), rect.top(), rect.right(), rect.bottom()))
        height = width = 128
        # im.show()
        ...
        uint_im = np.array(im, dtype=np.uint8).reshape((width, height))
        win.clear_overlay()
        win.set_image(uint_im)

        shape = predictor(uint_im, dlib.rectangle(0, 0, 128, 128))
        win.add_overlay(shape)
        dlib.hit_enter_to_continue()
        win.clear_overlay()
        shape = shape_to_np(shape)
        ...
        '''
                Third part - Drawing shapes from the face given
                This part took a decent chunk of time trying to actually draw them. 
                One reason being is that the image library needs numpy to be updated for scipy-image to work...
        '''
        ...
        new_im = np.zeros((width, height), dtype=np.uint8)

        new_im = draw_shape(new_im, jaw)
        new_im = draw_shape(new_im, right_eyebrow)
        new_im = draw_shape(new_im, left_eyebrow)
        new_im = draw_shape(new_im, nose)
        new_im = draw_shape(new_im, right_eye)
        new_im = draw_shape(new_im, left_eye)
        new_im = draw_shape(new_im, outer_mouth)
        new_im = draw_shape(new_im, inner_mouth)

        new_im = new_im.reshape(width, height)
        new_im = np.rot90(new_im)
        new_im = np.rot90(new_im)
        new_im = np.rot90(new_im)

        win.set_image(new_im)
        dlib.hit_enter_to_continue()
...
def main():
    ...
    '''
        This will demo how the preprocessing action works
        will demonstrate how it grabs the features and how we draw the shape of the face after cropping and everything
        requires you to hit enter to continue at each step
        Don't spam enter! Trust me it sometimes takes a second if you tabbed out and back into this and it registers every enter press...
    '''
    demo_preprocessing()
    ...
    '''
        # TODO: If you want to create training models this part has to be uncommented!!
    '''
    ...
    # ALL UNCOMMENTED BELOW WERE OVERFITTED
    #
    # num_epoch      = 10
    # batch_size     = 10
    # learning_rate  =  0.001
    ...
    '''
        Going to comment the three creates below so that it will only test with the preprocessed training data provided
        If you want to create the models uncomment the code below
    '''
